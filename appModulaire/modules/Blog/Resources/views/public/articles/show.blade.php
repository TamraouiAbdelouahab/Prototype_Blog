...
<div class="container mx-auto my-12 px-6">
    ...
    <div class="bg-gray-50 shadow-lg rounded-lg p-8 mt-12">
        ...
        @else
            <div class="mt-6 space-y-6">
                @foreach ($article->comments as $comment)
                    <div class="flex space-x-4 border-b border-gray-300 pb-4 mb-2">
                        <!-- User Avatar -->
                        <img class="w-12 h-12 rounded-full object-cover"
                            src="{{ $comment->user->profile_picture ?? 'https://ui-avatars.com/api/?name=' . urlencode($comment->user->name) . '&background=random&color=fff&meduim=true&size=80&length=2' }}"
                            alt="{{ $comment->user->name }}">
                        <div class="flex-1">
                            <!-- User Name and Timestamp -->
                            <div class="flex justify-between items-center">
                                <b class="ml-1  text-sm font-bold text-black">{{ $comment->user->name }}</b>
                                <p class="text-sm text-gray-500">{{ $comment->created_at->diffForHumans() }}</p>
                            </div>

                            <!-- Comment Content -->
                            <p class="mt-1 ml-2 text-gray-700">{{ $comment->content }}</p>
                        </div>
                    </div>
                @endforeach
            </div>
        ...
        <form action="{{ route('public.article.comments.store', $article->id) }}" id="comment-form" method="POST" class="mt-8">
            ...
            @enderror
            <div class="h-full flex" style="justify-content: flex-end">
                <button type="submit" class="mt-4 px-4 py-3 text-white bg-gray-800 hover:bg-gray-900 focus:outline-none focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm  me-2 mb-2 dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-700 dark:border-gray-700">
                    Add Comment
                </button>
            </div>
        </form>
